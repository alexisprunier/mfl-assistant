...
async def main(db, mail):
    logger.critical("compute_club_notifications - Start")
    ...
    logger.critical(f"compute_club_notifications - Number of listing/sale scopes to treat: {len(listing_scopes)}/{len(sale_scopes)}")

    # Treat listing scopes
    listings = await _get_listings_to_treat(db)
    logger.critical(f"compute_club_notifications - Number of listings to treat: {len(listings)}")
    ...
    logger.critical(f"compute_club_notifications - Number of sales to treat: {len(sales)}")
    ...
    if len(sales) > 0:
        ...
        for scope in sale_scopes:
            ...
            if len(club_ids) > 0:
                ...
                if len(user) > 0:
                    logger.critical(f"compute_club_notifications - Sale notification to send with {len(club_ids)} clubs")
                    user = user.pop()
                    notification = await _add_notification_in_db(db, scope["_id"], club_ids)
