...
const PopupAddTeam: React.FC<PopupAddTeamProps> = ({ trigger, onClose }) => {
  ...
  return (
    <div className="PopupAddTeam">
      <Popup trigger={trigger} modal closeOnDocumentClick onClose={onClose} className={"fade-in popup-md"}>
        {(close) => (
          <div>
            <div className="d-flex flex-row mb-3">
              <div className="flex-grow-1">
                <h2 className="text-white">Add a new team</h2>
              </div>
            ...
            <div className="d-flex flex-row justify-content-end mt-3">
              ...
              />
              <div>
                <button className="btn btn-info text-white" disabled={name.length < 3} onClick={() => confirm(close)}>
                  Confirm
                </button>
              </div>
            </div>
